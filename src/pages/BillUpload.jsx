...
OUTPUT: Return ONLY valid JSON (no markdown, no explanation):
{
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // STEP 1: Save to Firebase only (single source of truth)
      let billId = null;

      if (!user?.uid) {
        throw new Error("User not authenticated. Cannot save bill.");
      }
      ...
      try {
        ...
        // STEP 2: Upload actual file to Firebase Storage (if file exists) - NON-BLOCKING
        if (file && billId) {
          ...
          uploadBillDocument(file, billId)
            ...
            .catch(storageError => {
              ...
              if (storageError.message?.includes("No document to update")) {
                console.warn("⚠️ Bill update failed (document may not be fully synced yet):", storageError);
                console.log("ℹ️ Bill was saved successfully - file metadata will be updated on next sync");
              } else {
        ...
        if (user.uid && billId) {
          ...
          // Send email reminder if bill is overdue or due soon - NON-BLOCKING
          sendBillUploadReminder({
            ...
            .then(result => {
              if (result.sent) {
                console.log(`✅ Reminder email sent - Urgency: ${result.urgency?.urgencyLevel}`, result);
              } else {
                console.log(`📋 No reminder needed - Bill not due soon (${result.urgency?.daysUntilDue} days)`, result);
              }
            })
            .catch(emailError => {
              console.warn('⚠️ Could not send email reminder:', emailError);
            });
        }
      } catch (error) {
        console.error('❌ Error saving bill to Firebase:', error);
        setNotification({
          message: `Firebase Error: ${error.message}. Bill save failed.`,
          type: 'error'
        });
        setLoading(false);
        return;
