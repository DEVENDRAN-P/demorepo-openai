  ...
  useEffect(() => {
    ...
    getUserBills(user.uid)
      .then(fetchedBills => {
        ...
        // Calculate stats
        const totalGST = fetchedBills.reduce((sum, bill) => sum + (bill.taxAmount || 0), 0);
        const costSavings = calculateCostSavings(fetchedBills);

        setStats({
          totalBills: fetchedBills.length,
          pendingFiling: fetchedBills.filter(b => !b.filed).length,
          totalGST: totalGST,
        ...
        for (let i = 0; i < 3; i++) {
          ...
          const monthBills = fetchedBills.filter(b => {
        ...
        const newReminders = [];
        if (fetchedBills.filter(b => !b.filed).length > 0) {
          newReminders.push({
            type: 'warning',
            title: t('gstr3b_filing_due'),
            message: t('unconfirmed_bills_msg', {
              count: fetchedBills.filter(b => !b.filed).length,
    ...
    // Listen for custom bill update event (Firebase realtime)
    const handleBillUpdated = (event) => {
      const billId = event.detail?.billId;
      if (billId) {
        // Refresh bills from Firebase when a new bill is added
        getUserBills(user.uid)
          .then(fetchedBills => {
            setBills(fetchedBills);
            const updatedTotalGST = fetchedBills.reduce((sum, bill) => sum + (bill.taxAmount || 0), 0);
            const updatedCostSavings = calculateCostSavings(fetchedBills);
