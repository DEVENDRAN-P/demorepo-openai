...
export const getBillReminderStatus = async (userId, billId) => {
  try {
    ...
    try {
      billSnapshot = await getDoc(billRef);
    } catch (firestoreError) {
      // Handle Firebase offline error gracefully
      if (
        firestoreError.code === "unavailable" ||
        firestoreError.message?.includes("offline")
      ) {
        console.warn("⚠️ Firebase offline - using cached data", firestoreError);
        return {
          hasReminder: false,
          offline: true,
          message:
            "Firebase temporarily offline - will retry when connection restored",
        };
      }
    ...
    // Get the last sent reminder email - with offline handling
    ...
    try {
      ...
      // Silently fail for reminders query - not critical
      console.warn("⚠️ Could not fetch reminders:", remindersError);
    }
    ...
    // Return safe default instead of error object
    return {
      hasReminder: false,
      error: error.message,
      offline: error.code === "unavailable",
    };
  }
};
