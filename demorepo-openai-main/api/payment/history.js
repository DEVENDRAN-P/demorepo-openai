...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 2. Fetch payment transactions logs using helper
    console.log(`🔍 Fetching billing statement logs for user ${uid}`);
    const paymentLogs = await dbHelper.getPaymentHistory(uid);
    console.log(`✅ Returned ${paymentLogs.length} billing records for user ${uid}`);
