...
export const updateUserBill = async (billId, updates) => {
  try {
    ...
    if (!billDoc.exists()) {
      console.warn(
        `⚠️ Bill ${billId} not found - document may not be synced yet`,
      );
      // Silently fail - bill was already saved
      return {
        success: true,
        skipped: true,
        message: "Bill exists but update skipped (not yet synced)",
      };
    }
    ...
    if (error.message?.includes("No document to update")) {
      console.warn(
        `⚠️ Could not update bill ${billId} (document not fully synced)`,
        error,
      );
      return {
        success: true,
        skipped: true,
        message: "Update deferred - document not ready",
      };
    }
