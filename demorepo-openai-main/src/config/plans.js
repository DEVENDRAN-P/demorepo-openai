/**
 * FRONTEND MIRROR of the central subscription configuration.
 *
 * The authoritative copy lives server-side at lib/plans.js — this file is a
 * display/UX mirror used for pricing page parity, progress bars and upgrade
 * prompts. Backend enforcement NEVER reads from here. A test verifies both
 * files stay in sync (prices, limits, entitlements).
 *
...
export const PLAN_LIMITS = {
  invoiceUploads: { free: 10, pro: 50, business: "fair_use" },
  aiExtractions: { free: 10, pro: 50, business: "fair_use" },
  documents: { free: 5, pro: 20, business: "fair_use" },
  reports: { free: 2, pro: 10, business: "unlimited" },
  aiInsights: { free: 3, pro: 20, business: "unlimited" },
};

export const MULTI_BUSINESS_LIMITS = {
  free: 1,
  pro: 2,
  business: 5,
};

// Feature matrix (mirrors lib/plans.js FEATURES). levels: available | basic |
// advanced | blocked. required: plan needed to unlock (null = all plans).
...
// ---------------------------------------------------------------------------
// Helpers (display/UX only — never an authorization source)
// ---------------------------------------------------------------------------

export const PLANS = ["free", "pro", "business"];

export const normalizePlan = (plan) =>
  PLANS.includes(plan) ? plan : "free";

export const resolveLimit = (plan, metric) => {
  const p = normalizePlan(plan);
  const value = (PLAN_LIMITS[metric] || {})[p];
  if (value === "unlimited" || value === "fair_use") return Number.MAX_SAFE_INTEGER;
  return typeof value === "number" ? value : Number.MAX_SAFE_INTEGER;
};

export const isFairUse = (plan, metric) => {
  const p = normalizePlan(plan);
  return (PLAN_LIMITS[metric] || {})[p] === "fair_use";
};

export const displayLimit = (plan, metric) => {
  const p = normalizePlan(plan);
  const value = (PLAN_LIMITS[metric] || {})[p];
...
export const featureLevel = (feature, plan) => {
  const f = FEATURES[feature];
  if (!f) return "blocked";
  return f.plans[normalizePlan(plan)] || "blocked";
};

export const featureAllowed = (feature, plan) =>
  featureLevel(feature, plan) !== "blocked";

export const requiredPlanFor = (feature) => {
  const f = FEATURES[feature];
