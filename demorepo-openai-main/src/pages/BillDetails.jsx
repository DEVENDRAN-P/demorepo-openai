    ...
    if (!bill) {
        return (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', flexDirection: 'column', gap: '20px', color: 'var(--text-primary)' }}>
                <p style={{ fontSize: '18px' }}>{t('billdetails_not_found')}</p>
                <button onClick={() => navigate('/')} className="btn btn-primary">← {t('billdetails_back_dashboard')}</button>
            </div>
        );
    }
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                    <button onClick={() => navigate('/')} className="btn btn-outline">← {t('billdetails_back_console')}</button>
                    {!editing && (
                        <button onClick={handleEdit} className="btn btn-primary" style={{ padding: '0.5rem 1.5rem' }}>
                            ✎ {t('billdetails_edit_metadata')}
                        </button>
                    )}
                </div>
                ...
                <div className="glass-panel" style={{ borderRadius: 'var(--radius-xl)', padding: '1.5rem', marginBottom: '2rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem', borderLeft: `6px solid ${mathConsistent && complianceIssues.length === 0 ? 'var(--success)' : 'var(--error)'}` }}>
                    <div>
                        <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-tertiary)', fontWeight: 700 }}>{t('billdetails_audit_assessment')}</span>
                        <h2 style={{ fontSize: '1.5rem', margin: '0.25rem 0 0 0', fontWeight: 800 }}>
                            {complianceIssues.length === 0 && mathConsistent 
                              ? `✓ ${t('billdetails_fully_compliant')}` 
                              : <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: 'var(--warning)' }}><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> {t('billdetails_flagged', { count: complianceIssues.length + (!mathConsistent ? 1 : 0) })}</span>}
                        </h2>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '1rem' }}>
                        <div style={{ textAlign: 'center', background: 'var(--bg-secondary)', padding: '0.5rem 1.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
                            <span style={{ fontSize: '0.625rem', color: 'var(--text-tertiary)', display: 'block', textTransform: 'uppercase' }}>{t('billdetails_vetting_confidence')}</span>
                            <strong style={{ fontSize: '1.1rem', color: 'var(--theme-secondary)' }}>{scoreRating}</strong>
                        </div>
                        <div style={{ textAlign: 'center', background: 'var(--bg-secondary)', padding: '0.5rem 1.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
                            <span style={{ fontSize: '0.625rem', color: 'var(--text-tertiary)', display: 'block', textTransform: 'uppercase' }}>{t('billdetails_classification')}</span>
                ...
                <div className="ocr-viewer-container" style={{ margin: '0 0 2rem 0' }}>
                    ...
                    <div className="glass-panel" style={{ borderRadius: 'var(--radius-xl)', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        <div style={{ display: 'flex', justifyBetween: 'space-between', alignItems: 'center' }}>
                            <h3 style={{ fontSize: '1rem', fontWeight: 800, margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <IconRobot /> {t('billdetails_ocr_overlay')}
                            </h3>
                            <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{t('billdetails_hover_hint')}</span>
                        </div>
                        ...
                        <div className="ocr-image-wrapper" style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-lg)' }}>
                            {bill.downloadUrl ? (
                                ...
                                <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-tertiary)' }}>
                                    ...
                                    <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.8rem' }}>{t('billdetails_no_document')}</p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="glass-panel" style={{ borderRadius: 'var(--radius-xl)', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        <h3 style={{ fontSize: '1.15rem', fontWeight: 800, margin: 0, borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>{t('billdetails_data_fields')}</h3>
                        
                        <div onMouseEnter={() => setHoveredField('supplierName')} onMouseLeave={() => setHoveredField(null)}>
                            <label style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.25rem' }}>{t('billdetails_supplier_name')}</label>
                            {editing ? (
                        ...
                        <div className="grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                            ...
                            <div>
                                <label style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.25rem' }}>{t('billdetails_hsn_code')}</label>
                                {editing ? (
                        ...
                        <div className="grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                            <div onMouseEnter={() => setHoveredField('invoiceNumber')} onMouseLeave={() => setHoveredField(null)}>
                                <label style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.25rem' }}>{t('billdetails_invoice_number')}</label>
                                {editing ? (
                            ...
                            <div onMouseEnter={() => setHoveredField('invoiceDate')} onMouseLeave={() => setHoveredField(null)}>
                                <label style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.25rem' }}>{t('billdetails_invoice_date')}</label>
                                {editing ? (
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
                            <div onMouseEnter={() => setHoveredField('amount')} onMouseLeave={() => setHoveredField(null)}>
                                <label style={{ fontSize: '0.625rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.25rem' }}>{t('billdetails_taxable_val')} (₹)</label>
                                {editing ? (
                            ...
                            <div onMouseEnter={() => setHoveredField('taxAmount')} onMouseLeave={() => setHoveredField(null)}>
                                <label style={{ fontSize: '0.625rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.25rem' }}>{t('billdetails_gst_tax')} (₹)</label>
                                {editing ? (
                            ...
                            <div onMouseEnter={() => setHoveredField('totalAmount')} onMouseLeave={() => setHoveredField(null)}>
                                <label style={{ fontSize: '0.625rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.25rem' }}>{t('billdetails_grand_total')} (₹)</label>
                                {editing ? (
                        ...
                        <div>
                            <label style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.25rem' }}>{t('billdetails_filing_status')}</label>
                            {editing ? (
                                <select value={editData.filed ? 'true' : 'false'} onChange={(e) => handleInputChange('filed', e.target.value === 'true')} style={{ width: '100%', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', padding: '0.5rem 0.75rem', borderRadius: 'var(--radius-md)', cursor: 'pointer' }}>
                                    <option value="false">{t('billdetails_pending_filing')}</option>
                                    <option value="true">{t('billdetails_filed_gstr1')}</option>
                                </select>
                            ) : (
                                <span className={`badge-premium ${bill.filed ? 'badge-excellent' : 'badge-average'}`}>
                                    {bill.filed ? `✓ ${t('billdetails_filed')}` : `⏳ ${t('billdetails_pending')}`}
                                </span>
                            )}
                        </div>
                        ...
                        {editing && (
                            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', marginTop: '1rem' }}>
                                <button onClick={handleCancel} className="btn btn-outline">{t('billdetails_cancel')}</button>
                                <button onClick={handleUpdate} className="btn btn-primary">{t('billdetails_save_changes')}</button>
                            </div>
                        )}
                ...
                <div className="grid" style={{ gridTemplateColumns: '1.5fr 1.5fr', gap: '2rem', marginBottom: '2rem' }}>
                    ...
                    <div className="glass-panel" style={{ borderRadius: 'var(--radius-xl)', padding: '1.75rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', fontSize: '0.85rem' }}>
                            <div style={{ padding: '0.75rem 1rem', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', borderLeft: `4px solid ${mathConsistent ? 'var(--success)' : 'var(--error)'}` }}>
                                <strong>{t('billdetails_math_check')}</strong>
                            ...
                            <div style={{ padding: '0.75rem 1rem', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', borderLeft: '4px solid var(--info)' }}>
                                <strong>{t('billdetails_ocr_check')}</strong>
                                <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                                    {fraudIndicators.length === 0 
                                      ? t('billdetails_no_anomalies') 
                            ...
                            <div style={{ padding: '0.75rem 1rem', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', borderLeft: `4px solid ${complianceIssues.length === 0 ? 'var(--success)' : 'var(--error)'}` }}>
                                <strong>{t('billdetails_gstin_verification')}</strong>
                                <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                    ...
                    <div className="glass-panel" style={{ borderRadius: 'var(--radius-xl)', padding: '1.75rem' }}>
                        <h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            💡 {t('billdetails_explainability_log')}
                        </h3>
                        
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', fontSize: '0.85rem' }}>
                            <div>
                                <strong style={{ textTransform: 'uppercase', fontSize: '0.675rem', color: 'var(--text-tertiary)' }}>{t('billdetails_auto_category')}</strong>
                            ...
                            <div>
                                <strong style={{ textTransform: 'uppercase', fontSize: '0.675rem', color: 'var(--text-tertiary)' }}>{t('billdetails_ai_summary')}</strong>
                                <p style={{ margin: '0.25rem 0 0 0', fontStyle: 'italic', color: 'var(--text-primary)' }}>"{aiSummary}"</p>
                            </div>

                            <div>
                                <strong style={{ textTransform: 'uppercase', fontSize: '0.675rem', color: 'var(--text-tertiary)' }}>{t('billdetails_recommended_action')}</strong>
                ...
                {reminderStatus && (
                    <div className="glass-panel" style={{ borderRadius: 'var(--radius-xl)', padding: '2rem', marginBottom: '2rem' }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', justifyBetween: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem', marginBottom: '1.25rem', gap: '1rem' }}>
                            <h3 style={{ fontSize: '1.1rem', fontWeight: 800, margin: 0 }}>📧 {t('billdetails_smart_reminders')}</h3>
                            ...
                            >
                                {sendingEmail ? t('billdetails_sending') : `🤚 ${t('billdetails_trigger_reminder')}`}
                            </button>
                        </div>
                        ...
                        {reminderHistory && reminderHistory.length > 0 && (
                            <div style={{ marginTop: '1.5rem' }}>
                                <strong style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--text-tertiary)', display: 'block', marginBottom: '0.75rem' }}>{t('billdetails_comm_log')}</strong>
